...
export default function Menu() {
  ...
  const tipoUrl = searchParams.get('tipo') // 'delivery' | 'retiro' | null
  ...
  const [tipoSeleccionado, setTipoSeleccionado] = useState(tipoUrl || (mesa ? 'mesa' : null))
  ...
  if (!tipoSeleccionado) return (
    <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', background: '#111111', fontFamily: "'Segoe UI', system-ui, sans-serif", color: 'white' }}>
      ...
      <div style={{ position: 'relative', height: 220, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 20px 20px', display: 'flex', alignItems: 'flex-end', gap: 14 }}>
          {local.logo_url && <img src={local.logo_url} alt={local.nombre} style={{ width: 64, height: 64, borderRadius: 14, objectFit: 'cover', border: '2px solid rgba(255,255,255,0.2)', flexShrink: 0 }} />}
          <div>
            <h1 style={{ color: 'white', margin: 0, fontSize: 22, fontWeight: 800 }}>{local.nombre}</h1>
      ...
      <div style={{ padding: '28px 20px' }}>
        <p style={{ margin: '0 0 20px', fontSize: 16, fontWeight: 700, color: 'white', textAlign: 'center' }}>¿Cómo querés recibir tu pedido?</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>

          <button onClick={() => setTipoSeleccionado('delivery')} style={{
            ...
            <div>
              <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'white' }}>Delivery</p>
              <p style={{ margin: '3px 0 0', fontSize: 13, color: '#666' }}>Te lo llevamos a tu dirección</p>
            </div>
          </button>

          <button onClick={() => setTipoSeleccionado('retiro')} style={{
            background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 16,
            padding: '20px 20px', cursor: 'pointer', textAlign: 'left',
            display: 'flex', alignItems: 'center', gap: 16, fontFamily: 'inherit',
          }}>
            <span style={{ fontSize: 36 }}>🏪</span>
            <div>
              <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'white' }}>Para retirar</p>
              <p style={{ margin: '3px 0 0', fontSize: 13, color: '#666' }}>Pasás a buscarlo al local</p>
            </div>
          </button>
      ...
      <div style={{ textAlign: 'center', padding: '32px 20px 40px', borderTop: '1px solid #222' }}>
        ...
        <a href="https://nimble-strudel-515f0a.netlify.app" target="_blank" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 10 }}>
          <svg width="28" height="28" viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg">
          ...
          <div style={{ textAlign: 'left' }}>
            <p style={{ margin: 0, fontSize: 15, fontWeight: 800, color: 'white' }}>Valmai</p>
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', background: '#111111', fontFamily: "'Segoe UI', system-ui, sans-serif", color: 'white' }}>
      ...
      <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', gap: 8 }}>
          {mesa && (
            <div style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', color: 'white', fontSize: 12, fontWeight: 600, padding: '5px 12px', borderRadius: 20, border: '1px solid rgba(255,255,255,0.15)' }}>
              🪑 Mesa {mesa}
            </div>
          )}
          {tipoSeleccionado === 'delivery' && (
            <div style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', color: '#f59e0b', fontSize: 12, fontWeight: 600, padding: '5px 12px', borderRadius: 20, border: '1px solid rgba(245,158,11,0.3)' }}>
              🛵 Delivery
            </div>
          )}
          {tipoSeleccionado === 'retiro' && (
            <div style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', color: '#22c55e', fontSize: 12, fontWeight: 600, padding: '5px 12px', borderRadius: 20, border: '1px solid rgba(34,197,94,0.3)' }}>
              🏪 Para retirar
            </div>
          )}
        </div>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 18px 20px', display: 'flex', alignItems: 'flex-end', gap: 14 }}>
          {local.logo_url && (
            <img src={local.logo_url} alt={local.nombre} style={{ width: 72, height: 72, borderRadius: 16, objectFit: 'cover', border: '2px solid rgba(255,255,255,0.2)', flexShrink: 0, background: '#222' }} />
          )}
          <div>
      ...
      <div style={{ padding: '14px 14px 0', background: '#111' }}>
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{ width: '100%', boxSizing: 'border-box', background: '#222', border: '1px solid #333', borderRadius: 12, padding: '11px 14px 11px 40px', fontSize: 14, color: 'white', outline: 'none', fontFamily: 'inherit' }}
          />
        </div>
      </div>
      ...
      {pedidoActivo && pedidoActivo.slug === slug && (
        <div onClick={() => navigate(`/${slug}/estado/${pedidoActivo.pedidoId}?mesa=${pedidoActivo.mesa || ''}`)}
          style={{ margin: '10px 14px 0', background: 'linear-gradient(135deg, #b91c1c, #7f1d1d)', borderRadius: 14, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>{pedidoActivo.mesa ? `Mesa ${pedidoActivo.mesa}` : ''} · Tocá para ver el estado</p>
            </div>
          </div>
      ...
      {/* ── CATEGORÍAS ── */}
      {!busqueda && (
        <div style={{ background: '#111', position: 'sticky', top: 0, zIndex: 10, borderBottom: '1px solid #222' }}>
            ...
            {categorias.map(cat => {
              ...
              return (
                <button key={cat.id} onClick={() => setCategoriaActiva(cat.id)} style={{
                  flexShrink: 0, background: activa ? color : 'transparent',
                  color: activa ? '#000' : 'rgba(255,255,255,0.6)',
                  border: activa ? 'none' : '1px solid #333', borderRadius: 20, padding: '7px 16px',
      ...
      {/* ── FAB CARRITO ── */}
      {cantidad > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, padding: '10px 14px 16px', background: 'linear-gradient(to top, #111 80%, transparent)', zIndex: 100 }}>
          <button
            onClick={() => navigate(`/${slug}/pedido?mesa=${mesa || ''}&tipo=${tipoSeleccionado}`)}
            style={{ width: '100%', background: color, color: '#000', border: 'none', borderRadius: 14, padding: '15px 16px', fontSize: 15, fontWeight: 800, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontFamily: 'inherit' }}>
            <span style={{ background: 'rgba(0,0,0,0.15)', borderRadius: 20, padding: '4px 12px', fontSize: 13, fontWeight: 700 }}>
    ...
    <div style={{ margin: '0 14px 10px', background: '#1e1e1e', borderRadius: 16, overflow: 'hidden', border: '1px solid #2a2a2a', opacity: producto.disponible ? 1 : 0.4 }}>
      <div style={{ display: 'flex', gap: 0 }}>
        <div style={{ flex: 1, padding: '14px 14px 14px 16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div>
            ...
            {producto.descripcion && (
              <p style={{ margin: '5px 0 0', fontSize: 12, color: '#888', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{producto.descripcion}</p>
            )}
          </div>
          <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <p style={{ margin: 0, fontWeight: 800, fontSize: 17, color: color }}>₲ {producto.precio.toLocaleString()}</p>
            {cantidad > 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#2a2a2a', borderRadius: 24, padding: '4px 8px' }}>
                <button onClick={() => quitar(producto.id)} style={{ width: 28, height: 28, borderRadius: '50%', border: `1.5px solid ${color}`, background: 'transparent', color: color, fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, lineHeight: 1 }}>−</button>
                <span style={{ fontWeight: 800, fontSize: 15, minWidth: 18, textAlign: 'center', color: 'white' }}>{cantidad}</span>
                <button onClick={() => producto.disponible && agregar(producto)} style={{ width: 28, height: 28, borderRadius: '50%', border: 'none', background: color, color: '#000', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, lineHeight: 1 }}>+</button>
              </div>
            ) : (
        ...
        {producto.imagen_url ? (
          ...
          <div style={{ width: 110, height: 110, flexShrink: 0, background: '#2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36 }}>🍽️</div>
        )}
      </div>
      {!producto.disponible && (
        <div style={{ background: '#2a0000', textAlign: 'center', padding: '5px', fontSize: 11, color: '#ef4444', fontWeight: 600 }}>⚠️ No disponible</div>
      )}
    </div>
  )
}
